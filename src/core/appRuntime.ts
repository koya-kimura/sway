...
import { ImageStore } from "../utils/image/imageStore"; // 画像ストア
import { VideoStore } from "../utils/video/videoStore";
...
export const createAppRuntime = (config?: Partial<AppConfig>): AppRuntime => {
  ...
  const loadAssets = async (p: p5): Promise<void> => {
    try {
      ...
      context.assets.imageStore = imageStore;
      context.assets.videoStore = videoStore;
    } catch (error) {
  ...
  return {
    ...
    drawFrame(p: p5): void {
      ...
      visualComposer.update(
        ...
        captureManager,
        visualFont,
        context.assets.imageStore,
        context.assets.videoStore,
      );
      visualComposer.draw(
        p,
        midiManager,
    ...
    handleKeyPressed(p: p5): void {
      ...
      context.assets.videoStore?.resumeAll();
    ...
    dispose(): void {
      ...
      context.assets.videoStore?.dispose();
    },
